...
- **Docker Integration Test Environment**: Complete Docker-based testing environment with real WordPress + MySQL instances. Provides two isolated WordPress environments (source and destination) for end-to-end migration testing. Includes Dockerfile, docker-compose.yml for orchestration, automated test runner script, and comprehensive documentation. Docker tests run in CI/CD to verify archive imports work in real WordPress environments. See `tests/docker/README.md` for details.
