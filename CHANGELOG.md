...
- **[v2 ONLY]** **HIGH**: Fixed cryptic "command not found" errors during adapter detection. Moved dependency checks before adapter detection logic. Previously, `detect_adapter()` would call validate functions that immediately use `file`, `unzip`, or `tar` commands. With `set -e` enabled, missing tools caused script to terminate with bare "command not found" before reaching the friendly "missing dependency" error in `check_adapter_dependencies()`. Now checks for `file` command and at least one archive tool (`unzip` or `tar`) before attempting detection, providing clear installation instructions if missing.
